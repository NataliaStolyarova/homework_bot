...
def create_mock_response_get_with_custom_status_and_data(
    ...
    def mocked_response(*args, **kwargs):
        response = utils.MockResponseGET(
            *args, random_timestamp=random_timestamp, http_status=http_status, **kwargs
        )
...
class TestHomework:
    ...
    def test_request_get_call(self, monkeypatch, current_timestamp, homework_module):
        func_name = "get_api_answer"
        utils.check_function(
            homework_module, func_name, self.HOMEWORK_FUNC_WITH_PARAMS_QTY[func_name]
        )

        def check_request_get_call(url, current_timestamp=current_timestamp, **kwargs):
